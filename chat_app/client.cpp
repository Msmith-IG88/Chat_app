...
public:
    ...
    {
        if (error) {
            std::cout << "Disconnected from session by host.\n";
        }
    }
...
{
    if (argc != 3) {
        std::cerr << "You must specify the host and service or port for the client to connect to.\n";
        return -1;
    }
    ...
    try {
        ...
        tcp_client<chat_client> client{io_context, chat_application, host, service_or_port};
