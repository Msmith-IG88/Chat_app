#ifndef CHAT_APP_TCP_HPP
#define CHAT_APP_TCP_HPP
...
{
public:
    static auto create(boost::asio::io_context& io_context, std::shared_ptr<T> application)
    {
        // construct shared_ptr instead of using make_shared to use private constructor
        return std::shared_ptr<tcp_connection>{new tcp_connection{io_context, application}};
    }
    ...
    void write(const CharIterable& write_body)
    {
        ...
        post(socket_.get_executor(), [this, self, write_body]() {
    ...
    void close(const boost::system::error_code& error)
    {
        ...
        post(socket_.get_executor(), [this, self, error]() {
...
private:
    tcp_connection(boost::asio::io_context& io_context, std::shared_ptr<T> application)
        : socket_{io_context}
...
{
public:
    tcp_server(boost::asio::io_context& io_context, std::shared_ptr<T> application, const uint16_t port)
...
private:
    void start_accept()
    {
        auto new_connection = tcp_connection<T>::create(io_context_, application_);
...
public:
    tcp_client(boost::asio::io_context& io_context,
               std::shared_ptr<T> application,
               const std::string& server,
               const std::string& service_name_or_port)
        : io_context_{io_context}
        , endpoint_resolver_{io_context}
        , application_{application}
    {
...
private:
    void connect(const std::string& server, const std::string& service_name_or_port)
    {
        ...
        boost::asio::async_connect(
            ...
            [this, new_connection](const boost::system::error_code& error, const boost::asio::ip::tcp::endpoint& endpoint) {
                if (!error) {
                    new_connection->start();
                }
            });
